...
df = df.dropna(subset=["gender"])  # Remove rows where gender is NaN

# Convert gender to integer (only if necessary)
df["gender"] = df["gender"].astype(int)  # Ensures it's an integer, but no mapping needed
